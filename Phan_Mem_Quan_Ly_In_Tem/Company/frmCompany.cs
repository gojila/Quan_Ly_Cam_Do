...
using Phan_Mem_Quan_Ly_In_Tem.Company;
...
namespace Phan_Mem_Quan_Ly_In_Tem
{
    public partial class frmCompany : XtraForm
    {
        ...
        public delegate void SelectedCompanyEventHander(string companyStandardCode, string companyName, string companyAddress);

        public event SelectedCompanyEventHander SelectedCompany;
        private void RaiseSelectedCompanyEventHander(string companyStandardCode, string companyName, string companyAddress)
        {
            if (SelectedCompany != null)
            {
                SelectedCompany(companyStandardCode, companyName, companyAddress);
            }
        }
        ...
        private void ActiveEditor_DoubleClick(object sender, EventArgs e)
        {
            if (opentype == "")
            {
                var companyStandardCode = gbList.GetFocusedRowCellValue(colCompanyStandardCode);
                var companyName = gbList.GetFocusedRowCellValue(colCompanyName);
                var companyAddress = gbList.GetFocusedRowCellValue(colCompanyAddress);

                RaiseSelectedCompanyEventHander(
                    companyStandardCode.ToString(),
        ...
        private void DanhSachKhachHang_Load(object sender, EventArgs e)
        {
            if (opentype == "edit") 
            {
                bbiXoa.Visibility = DevExpress.XtraBars.BarItemVisibility.Always;
            }
        }
        ...
        private void bbiXem_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            gcList.DataSource = db.Companies.Where(s => !(s.IsDeleted ?? false)).ToList();
        ...
        */
        ...
        private void bbiSua_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                var id = gbList.GetFocusedRowCellValue(colCompanyID.FieldName);
                if (id != null)
                {
                    var _frmAdd = new frmCompanyAdd(id.ToString());
                    _frmAdd.Reload += () =>
                    {
                        bbiXem_ItemClick(this, null);
                    };
        ...
        private void bbiXoa_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (selectedRows.Length > 0)
                {
                    ...
                    {
                        var arg = gbList.GetRowCellValue(selectedRows[i - 1], colCompanyID);
                        if (arg == null)
                            continue;
                    ...
                    if (!string.IsNullOrEmpty(ids))
                    {
                        ...
                        {
                            db.Database.ExecuteSqlCommand("UPDATE [Company] SET [IsDeleted] = 1, [DeletedDate] = GETDATE() WHERE [CompanyID] IN (" + ids + ")");
                            db.SaveChanges();
                        }
                    }
